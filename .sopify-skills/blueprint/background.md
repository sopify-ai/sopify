...
- **已完成**：ActionProposal P0 thin slice（ADR-017）、archive lifecycle cutover、legacy feature cleanup、P1 subject identity binding
...
- **Protocol 外部入口缺口**：Protocol-first 是确认的架构方向，Convention 最小消费性 smoke 已跑通（read + continue，不含 finalize/archive/blueprint 回写），但仓库默认产品入口（README / install / quickstart）仍偏 runtime-first。外部宿主开发者尚无 non-runtime quickstart 可用

### 生态横向定位（2026-05 分析）

2026-05 对 6 个同领域项目做横向分析后，Sopify 的生态位更加清晰：

| 方向 | 代表 | Sopify 的关系 |
|------|------|-------------|
| AI CLI 工作流编排 | HelloAGENTS | Sopify 不做编排，但 HelloAGENTS 的交付证据链（contract.json / review.json）启发了 Verifier evidence 标准化方向 |
| 通用长期代理平台 | Hermes Agent | Sopify 不做 agent platform；Hermes 的知识自动提炼方向作为 T0 Reference 记录 |
| Repo-native 规格系统 | OpenSpec | Sopify 不做 spec 方法论；OpenSpec 的 delta 语义（ADDED/MODIFIED/REMOVED）启发了 ActionProposal side_effect 结构化方向 |
| SDD 分发与扩展平台 | Spec-Kit | Sopify 不做 skill 分发/marketplace；Spec-Kit 的工作流引擎和离线分发作为 T0 Reference |
| 编码代理行为纪律 | Superpowers | Sopify 不做方法论教学；Superpowers 的 headless behavioral verification 启发了 Protocol Compliance Suite 方向 |

**Sopify 的不可替代面不在某一项功能，而在可验证的便携式证据与授权语义**——这一判断在横向分析后得到强化。当前最大的潜力不是"再多支持几个宿主"，而是先证明自己是一个宿主无关、可被独立消费和独立验证的协议层。
