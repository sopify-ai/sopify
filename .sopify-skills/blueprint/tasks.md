...
- [ ] Convention 入口兑现差距：Convention 下界已在 protocol.md formalize 并通过 smoke，但仓库默认产品入口（README / install / quickstart）仍偏 runtime-first；在将 Convention 作为外部主入口前，需补足 validator-first onboarding、non-runtime quickstart 与配套 smoke
