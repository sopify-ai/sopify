# 技术设计: Plan A 子计划 | 风险自适应打断与局部语义分类收敛

## 设计目标

本子 plan 的设计目标不是立刻交付代码，而是把 Plan A 的实现前提收敛成一套可执行的设计骨架：
...
7. 默认与 B1 已发布 contract 保持兼容

## 问题重述

Plan A 真正要分类的对象，不是“整句用户话术属于哪个词槽”，而是：
...
因此，Plan A 的核心设计对象应该是“action-in-context classifier”，而不是“全局意图分类器”。
...
1. 目录名、`plan_id`、`feature_key` 统一采用中性、能力导向命名。
2. 禁止使用可直接暴露来源关联关系的命名锚点。
3. 命名改动必须一次性联动 metadata 与正文引用，避免残留单点泄露。
...
1. 对当前方案包执行来源锚点 denylist 扫描并要求零命中。
2. 校验范围覆盖目录名、`plan_id`、`feature_key`、正文、图示标签。

对外流通约束：`doc1-governance` 合并前，本方案包仅限内部使用，不得向公司外部或外部合作方流通。
...
3. `analysis-only` 改道 `consult_readonly` 已拍板并进入首批样例行；其余产品分叉继续按”待拍板问题”管理。
...
1. `Plan B1` contract 已稳定，且 parser-first v1 已通过当前 acceptance gate
...
4. 把 semantic side classifier 保留为 Plan A 的二阶段增强选项
...
| 顺序 | 建议分支 | 承载任务 | 目标 |
| --- | --- | --- | --- |
| 1 | `feature/plan-a-boundary-core` | `1.1-1.3`、`2.1-2.2`、`9.1-9.7`、`10.1-10.6`、`11.1-11.3` | 冻结 V1 的边界、fail-close contract、三张表结构与失败矩阵 |
| 2 | `feature/plan-a-doc1-governance` | `8.1-8.4` | 并行完成公开发布面的去显性来源化治理 |
| 3 | `feature/plan-a-v1-guard-rails`（4a） | `3.1-3.4`、`4.1-4.3`、`17.1-17.3`、`18.1-18.6`、`19.1-19.5` | 先落物理防线：范围守卫、真理表资产、模板安全、legacy quarantine 出口 |
| 4 | `feature/plan-a-sample-invariant-gate` | `5.1-5.3`、`6.1-6.6`、`16.1-16.5` | 用 A-1~A-8（含历史错例）完成三张表可填平验证与状态不变量断言 |
| 5 | `feature/plan-a-v1-scope-finalize`（4b） | `7.1-7.4`、`17.4` | 基于样本压测结果锁定 v1 file map/模块白名单与 rollout/rollback 口径 |
| 6 | `feature/plan-a-vnext-gate` | `4.4`、`12.1-12.4`、`13.1-13.4` | 冻结 vNext 升级条件、预算门槛与回滚信号（不把 classifier 主链上线） |

依赖关系：

1. `boundary-core` 先行。
2. `doc1-governance` 可与 `boundary-core` 并行。
...
进入条件：`feature/plan-a-boundary-core + feature/plan-a-v1-guard-rails` 准备合并。

必填决策：

1. `plan_proposal_pending + command prefix` 的最终动作口径（保持 fail-close 或产品调整）。
2. A-6 归属（全部归 Plan A 或拆为 contract-safety 子项）。
...
进入条件：`feature/plan-a-sample-invariant-gate` 准备合并。
...
进入条件：`feature/plan-a-v1-scope-finalize` 准备合并。

必填决策：

1. v1 implementation file map 与模块白名单冻结。
2. 运行态范围锁死骨架完成（常量注册表 + 守卫测试），超范围改动必须失败。
3. rollout/rollback 与 B1 compatibility 口径冻结。
...
进入条件：`feature/plan-a-vnext-gate` 准备合并。
...
| # | 问题 | 阻断层级 | 当前状态 | 最晚关闭时机 |
|---|------|---------|---------|------------|
| 1 | `plan_proposal_pending + command prefix`：是否属于产品行为调整，还是保持现状并显式 fail-close | Checkpoint A（阻断 V1） | open | Checkpoint A 进入前 |
| 2 | A-6 归属：全部归 Plan A，还是拆出单独的 contract-safety 子项 | Checkpoint A/B（阻断 V1） | open | Checkpoint A 进入前 |

**A-6 当前工作假设：** 留在 Plan A（v1 gate 成员，见 §Phase 1 冻结口径）。若最终拆出，需同步修订：§Phase 1 v1 gate 成员列表、§Checkpoint B 第 3 条、§Acceptance Gate 第 1 条。
