...
4. 当前公开口径评审结论：目录名、`plan_id`、`feature_key`、正文术语、图示标签需统一去显性来源锚点
...
- `runtime/engine.py`
- 相关 `tests/` 与回归样本
- 当前方案包的公开命名与术语治理（目录名、`plan_id`、`feature_key`、正文、图示标签）
...
- 将“私有研究映射”与“公开抽象原则”分层管理：公开层只保留机制与约束，私有层保留具体映射笔记
- 对目录名、`plan_id`、`feature_key`、正文与图示标签执行统一中性命名收敛
- 对来源锚点执行一次性 denylist 扫描，要求零命中后再进入对外分享
